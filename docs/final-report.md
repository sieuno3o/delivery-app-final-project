...
- **주문과 주문상세:** 한 주문에는 메뉴가 여러 개이므로 배송지·총액 같은 공통값은 `orders`, 반복되는 메뉴명·단가·수량은 `order_items`로 분리했다. `order_items`에 주문 당시 메뉴명과 단가를 복사해 메뉴가 수정·삭제되어도 과거 주문 기록은 바뀌지 않는다.
...
UUID 중복 방지 키의 고유 제약으로 버튼을 여러 번 눌러도 주문은 한 건만 남는다. 주문 조회에는 주문 ID와 현재 로그인한 `user_id`를 함께 사용해 다른 사용자의 주문을 볼 수 없게 했다. 로컬은 Docker Compose의 PostgreSQL, 운영은 Neon PostgreSQL을 사용하며 같은 Drizzle 마이그레이션으로 스키마를 맞췄다.
...
| 영역 | 검증 내용 | 결과 |
| --- | --- | ---: |
| 필수 기능 | 회원가입 · 로그인 · 로그아웃 · 식당/메뉴 · 장바구니 · 주문 저장 · 내 주문내역 | **7 / 7** |
| 추가 기능 | 검색·카테고리/배달비/최소 주문 필터 · 품절 처리 · 상태 타임라인 · 관리자 상태 변경 · 모바일 UI | 구현 |
